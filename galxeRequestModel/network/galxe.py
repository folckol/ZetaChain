...
class Galxe(NetworkHelper):
    ...
    def verif(self, campaign_id, credentials):
        for i in credentials:
            cap_data = self.solve_captcha(campaign_id)
            # logger.info(cap_data)
            payload = {"operationName": "AddTypedCredentialItems",
                       "variables":
                           {"input":
                                {"credId": i,
                                 "operation": "APPEND",
            ...
            with self.session.post('https://graphigo.prd.galaxy.eco/query', json=payload, timeout=10) as response:
                logger.success(f"{self.index}: success {credentials.index(i)}")

    def parse_points(self):
        payload = {"operationName": "SpaceBasicWithAuthQuery",
                   "variables": {"alias": "polyhedra",
                                 "address": "0xff7742811497a66130585ab96b2870073fbbc207",
                                 "firstTailoredTask": 10,
                                 "afterTailoredTask": "-1"},
                   "query": "query SpaceBasicWithAuthQuery($id: Int, $alias: String, $address: String!, $firstTailoredTask: Int, $afterTailoredTask: String) {\n  space(id: $id, alias: $alias) {\n    ...SpaceBasic\n    ...SpaceTailoredTasksWithAuth\n    isFollowing\n    isAdmin(address: $address)\n    unclaimedBackfillLoyaltyPoints(address: $address)\n    addressLoyaltyPoints(address: $address) {\n      id\n      points\n      rank\n      __typename\n    }\n    __typename\n  }\n}\n\nfragment SpaceBasic on Space {\n  id\n  name\n  thumbnail\n  alias\n  isVerified\n  info\n  links\n  status\n  followersCount\n  followersRank\n  backers\n  categories\n  token\n  discordGuildID\n  discordGuildInfo\n  __typename\n}\n\nfragment SpaceTailoredTasksWithAuth on Space {\n  tailoredTasks(first: $firstTailoredTask, after: $afterTailoredTask) {\n    totalCount\n    pageInfo {\n      hasNextPage\n      __typename\n    }\n    list {\n      id\n      spaceId\n      title\n      description\n      link\n      tag\n      points\n      startTime\n      claimedPoints(address: $address)\n      __typename\n    }\n    __typename\n  }\n  __typename\n}\n"}
        with self.session.post('https://graphigo.prd.galaxy.eco/query', json=payload, timeout=10) as response:
            logger.info(f"{self.index}: {response.json()['data']['space']['addressLoyaltyPoints']['points']}")

    def claim(self, campaign_id):
        self.authorize()
        ...
        try:
            ...
            with self.session.post('https://graphigo.prd.galaxy.eco/query', json=payload, timeout=10) as response:
                ...
                if data['allow']:
                    logger.success(f'{self.index}: {campaign_id} | Successfully claimed')
                else:
                    logger.error(f'{self.index}: {campaign_id} | {data["disallowReason"]}')
        except:
            # traceback.print_exc()
            logger.error(f'{self.index}: {campaign_id} | Claim error')


    def solve_captcha(self, camp_id='GCDTWUWt4p'):
        try:
            ...
            payload = {
                ...
                "pageurl": f"https://galxe.com/polyhedra/campaign/{camp_id}",
                "json": 1
            }
